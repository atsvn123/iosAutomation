...
class zxtouch:
    ...
    def tap_text(self, text, region=None, exact=False, case_sensitive=False, occurrence=0,
        ...
        target = str(text) if case_sensitive else str(text).lower()
        matched_count = 0

        for item in results:
            candidate = item.get("text", "")
            candidate_to_match = candidate if case_sensitive else candidate.lower()
    ...
    def wait_for_text(self, text, timeout=10, interval=0.5, region=None, exact=False, case_sensitive=False,
                      occurrence=0, custom_words=[], minimum_height="", recognition_level=0, languages=[],
                      auto_correct=0, debug_image_path=""):
